    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">
            <a href="{{ route('managers.index') }}" class="text-white hover:text-gray-400">
                {{ __('managers.create.title') }}
            </a>
            <span class="mx-2"> &gt; </span>
            {{ __('managers.create.create') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-md rounded-lg p-6">
                <form action="{{ route('managers.store') }}" method="POST">
                    ...
                    <!-- Name Field -->
                    <div class="mb-4">
                        <label for="name" class="form-label">{{ __('managers.create.name') }}</label>
                    ...
                    <!-- Role Field -->
                    <div class="mb-4">
                        <label for="role" class="form-label">{{ __('managers.create.role') }}</label>
                    ...
                    <!-- Email Field -->
                    <div class="mb-4">
                        <label for="email" class="form-label">{{ __('managers.create.email') }}</label>
                    ...
                    <!-- Phone Field -->
                    <div class="mb-4">
                        <label for="phone" class="form-label">{{ __('managers.create.phone') }}</label>
                    ...
                    <div class="d-flex justify-content-end mt-4">
                        <a href="{{ route('managers.index') }}" class="ml-4 px-4 py-2 text-gray-600 border border-gray-300 border-2 rounded-md focus:outline-none no-underline">
                            {{ __('managers.create.cancel') }}
                        </a>

                        <button class="btn btn-primary ml-4 px-4 py-2">
                            {{ __('managers.create.save') }}
                        </button>
                    </div>
                </form>
            </div>
