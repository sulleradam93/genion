    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">
            <a href="{{ route('company_jobs.index') }}" class="text-white hover:text-gray-400">
                {{ __('company_jobs.edit.positions') }}
            </a>
            <span class="mx-2"> &gt; </span>
            {{ __('company_jobs.edit.edit_position') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-md rounded-lg p-6">
                <form action="{{ route('company_jobs.update', $companyJob) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <input type="hidden" name="company_id" id="company_id" value="{{ old('job_id', $companyJob->company_id) }}">
                    <input type="hidden" name="job_id" id="job_id" value="{{ old('job_id', $companyJob->job_id) }}">

                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="company" class="form-label">{{ __('company_jobs.edit.select_company') }}</label>
                            <div class="autocomplete-group">
                                <input type="text" id="company" class="autocomplete-input form-control @error('company_id') is-invalid @enderror"
                                    autocomplete="off"
                                    value="{{ old('company_name', $companyJob->company_name) }}"
                                    data-id="{{ old('company_id', $companyJob->company_id) }}">
                        ...
                        <div class="col-md-6">
                            <label for="job" class="form-label">{{ __('company_jobs.edit.select_job') }}</label>
                            <div class="autocomplete-group">
                                <input type="text" id="job" class="autocomplete-input form-control @error('job_id') is-invalid @enderror"
                                    ...
                                    data-id="{{ old('job_id', $companyJob->job_id) }}">
                    ...
                    <div class="mb-4">
                        <label for="base_salary" class="form-label">{{ __('company_jobs.edit.base_salary') }}</label>
                        <input type="number" name="base_salary" id="base_salary" class="form-control @error('base_salary') is-invalid @enderror" value="{{ old('base_salary', $companyJob->base_salary) }}" required>
                        @error('base_salary')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        <label for="night_salary" class="form-label">{{ __('company_jobs.edit.night_salary') }}</label>
                        <input type="number" name="night_salary" id="night_salary" class="form-control @error('night_salary') is-invalid @enderror" value="{{ old('night_salary', $companyJob->night_salary) }}">
                        @error('night_salary')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        <label for="weekend_salary" class="form-label">{{ __('company_jobs.edit.weekend_salary') }}</label>
                        <input type="number" name="weekend_salary" id="weekend_salary" class="form-control @error('weekend_salary') is-invalid @enderror" value="{{ old('weekend_salary', $companyJob->weekend_salary) }}">
                    ...
                    <div class="d-flex justify-content-end mt-4">
                        <a href="{{ route('company_jobs.index') }}" class="ml-4 px-4 py-2 text-gray-600 border border-gray-300 border-2 rounded-md focus:outline-none no-underline">
                            {{ __('company_jobs.edit.cancel') }}
                        </a>

                        <button class="btn btn-primary ml-4 px-4 py-2">
                            {{ __('company_jobs.edit.save') }}
                        </button>
                    </div>
                </form>
            </div>
...
$(document).ready(function () {
    ...
    $('#base_salary').on('keyup', function () {
        ...
        $('#night_salary, #weekend_salary').val(baseValue);
    });
